...
function product(P::AbstractProjMPO, v::ITensor)::ITensor
  ...
  if P.product_label==2
     if P.nsite==2
         push!(itensor_map, prime(P.H[P.lpos + 1]))
         push!(itensor_map, prime(P.H[P.rpos - 1]))
     else
         push!(itensor_map, prime(P.H[P.lpos + 1]))
     end
  end
...
function position2!(P::AbstractProjMPO,
                   psi::MPS,
                   pos::Int)
  makeL2!(P,psi,pos-1)
  makeR2!(P,psi,pos+nsite(P))
...
function makeL2!(P::AbstractProjMPO,
                psi::MPS,
                k::Int)
  while P.lpos < k
    ll = P.lpos
    if ll <= 0
      P.LR[1] = psi[1]*P.H[1]*prime(P.H[1])*dag(prime(prime(psi[1],"Site")))
      P.lpos = 1
    else
      P.LR[ll+1] = P.LR[ll]*psi[ll+1]*P.H[ll+1]*prime(P.H[ll+1])*dag(prime(prime(psi[ll+1],"Site")))
      P.lpos += 1
    end
  end
end
...
function makeR2!(P::AbstractProjMPO,
                psi::MPS,
                k::Int)
  N = length(P.H)
  while P.rpos > k
    rl = P.rpos
    if rl >= N+1
      P.LR[N] = psi[N]*P.H[N]*prime(P.H[N])*dag(prime(prime(psi[N],"Site")))
      P.rpos = N
    else
      P.LR[rl-1] = P.LR[rl]*psi[rl-1]*P.H[rl-1]*prime(P.H[rl-1])*dag(prime(prime(psi[rl-1],"Site")))
      P.rpos -= 1
    end
  end
end
...
function position3!(P::AbstractProjMPO,
                   psi::MPS,
                   phi::MPS,
                   pos::Int)
  makeL3!(P,psi,phi,pos-1)
  makeR3!(P,psi,phi,pos+nsite(P))
...
function makeL3!(P::AbstractProjMPO,
                psi::MPS,
                phi::MPS,
                k::Int)
  while P.lpos < k
    ll = P.lpos
    if ll <= 0
      P.LR[1] = psi[1]*P.H[1]*dag(prime(phi[1]))
      P.lpos = 1
    else
      P.LR[ll+1] = P.LR[ll]*psi[ll+1]*P.H[ll+1]*dag(prime(phi[ll+1]))
      P.lpos += 1
    end
  end
end
...
function makeR3!(P::AbstractProjMPO,
                psi::MPS,
                phi::MPS,
                k::Int)
  N = length(P.H)
  while P.rpos > k
    rl = P.rpos
    if rl >= N+1
      P.LR[N] = psi[N]*P.H[N]*dag(prime(phi[N]))
      P.rpos = N
    else
      P.LR[rl-1] = P.LR[rl]*psi[rl-1]*P.H[rl-1]*dag(prime(phi[rl-1]))
      P.rpos -= 1
    end
  end
end
...
function position4!(P::AbstractProjMPO,
                   psi::MPS,
                   phi::MPS,
                   pos::Int)
  makeL4!(P,psi,phi,pos-1)
  makeR4!(P,psi,phi,pos+nsite(P))
...
function makeL4!(P::AbstractProjMPO,
                psi::MPS,
                phi::MPS,
                k::Int)
  while P.lpos < k
    ll = P.lpos
    if ll <= 0
      P.LR[1] = psi[1]*P.H[1]*prime(P.H[1])*dag(prime(prime(phi[1],"Site")))
      P.lpos = 1
    else
      P.LR[ll+1] = P.LR[ll]*psi[ll+1]*P.H[ll+1]*prime(P.H[ll+1])*dag(prime(prime(phi[ll+1],"Site")))
      P.lpos += 1
    end
  end
end
...
function makeR4!(P::AbstractProjMPO,
                psi::MPS,
                phi::MPS,
                k::Int)
  N = length(P.H)
  while P.rpos > k
    rl = P.rpos
    if rl >= N+1
      P.LR[N] = psi[N]*P.H[N]*prime(P.H[N])*dag(prime(prime(phi[N],"Site")))
      P.rpos = N
    else
      P.LR[rl-1] = P.LR[rl]*psi[rl-1]*P.H[rl-1]*prime(P.H[rl-1])*dag(prime(prime(phi[rl-1],"Site")))
      P.rpos -= 1
    end
  end
end
...
function product2(P::AbstractProjMPO, v::ITensor)::ITensor
  itensor_map = Union{ITensor,OneITensor}[lproj(P)]
  append!(itensor_map, P.H[site_range(P)])
  push!(itensor_map, rproj(P))
...
function product3(P::AbstractProjMPO, v::ITensor)::ITensor
  itensor_map = Union{ITensor,OneITensor}[lproj(P)]
  append!(itensor_map, P.H[site_range(P)])
  if P.nsite==2
      push!(itensor_map, prime(P.H[P.lpos + 1]))
