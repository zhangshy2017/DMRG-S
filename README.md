...
`abstractprojmpo.jl` adds some new methods to contract local tensors between the MPO $(H-\xi)^2$ and MPS $|psi\rangle$ in order to obtain $\mathcal{A}_{t,\text{eff}}^{[i,i+1]}$  and $\tilde{\psi}_{t-1}^{[i,i+1]}$ [<sup>2</sup>](#refer-anchor-1). The original function "product" is overloaded to implement the opreration of the matrix $\mathcal{A}_{t,\text{eff}}^{[i,i+1]}$ mutiplying a vector.
